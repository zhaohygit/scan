            ...
            console.log(DWObject)      
             cos.putObject({
                ...
                if(data.statusCode == 200){
                	...
                	var url = 'https://zhihuotech.com/insight_dev/paper/save_pictures'
									...
									$.ajax({
										...
										success:function(e){
											...
											if(e.data.status == 1){
												...
												$('.zs_num').text(e.data.all_points)
												$('.student_name').text(e.data.student_name)
												
												// 知识点
												e.data.question_corrects.map(item=>{
													var trTD = `<ul class="topic">
																	<li style="flex:81;margin-left: 20px;">${item.question_num}</li>
																	<li style="flex:291">${item.point_name}</li>
																	<li style="flex:190">${item.right_ans}</li>
																	<li style="flex:190;color:${item.right_or_wrong==1?'rgba(0, 0, 0, 1)':'rgba(255, 59, 48, 1)'}" >${item.student_ans}</li>
																	<li style="flex:190;display:flex;justify-content:center;align-items:center;">
																		<img src="Images/icon_true.png" style="width:23px;height:16px;${item.right_or_wrong==0?'display:none':'display:block'}">    
																		<img src="Images/icon_false.png" style="width:16px;height:16px;${item.right_or_wrong==0?'display:block':'display:none'}">    
																	</li>
																</ul>`
